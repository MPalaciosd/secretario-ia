...
async function createEvent(userId, data) {
  ...
  if (conflictCheck.rows.length > 0) {
    ...
    throw new Error(
      `Conflicto de horario con "${conflict.title}" a las ${conflictTime.toLocaleTimeString('es-ES', {hour: '2-digit', minute: '2-digit'})}. ¿Quieres programarlo de todas formas?`
    );
  }
  
  const result = await query(
    `INSERT INTO events (user_id, title, description, start_time, end_time, duration_minutes, event_type, metadata)
...
async function updateEvent(userId, eventId, data) {
  ...
  if (data.date && data.time) {
    const newStartTime = new Date(`${data.date}T${data.time}:00`);
    updateFields.push(`start_time = $${paramIndex}`); values.push(newStartTime.toISOString()); paramIndex++;
    
    if (data.duration_minutes || existing.rows[0].duration_minutes) {
      const duration = data.duration_minutes || existing.rows[0].duration_minutes;
      const newEndTime = new Date(newStartTime.getTime() + duration * 60 * 1000);
      updateFields.push(`end_time = $${paramIndex}`); values.push(newEndTime.toISOString()); paramIndex++;
    }
  }
  
  if (data.duration_minutes) {
    updateFields.push(`duration_minutes = $${paramIndex}`); values.push(data.duration_minutes); paramIndex++;
  }
