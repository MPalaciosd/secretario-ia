...
function startServer() {
  ...
  app.use(cors({
    origin: (origin, callback) => {
      // Allow no-origin requests only in development (curl, Postman, etc.)
  ...
  app.get('/api/subscription', require('./middleware/auth').authMiddleware, async (req, res, next) => {
    try {
      ...
      // Fallback — return cached status from JWT if Stripe call fails
      try {
        const { getPlan } = require('./middleware/subscription');
        const plan = getPlan(req.user.subscription_status);
