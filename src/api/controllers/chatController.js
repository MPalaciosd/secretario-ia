...
function buildMissingDataQuestion(missingFields) {
  ...
  const questions = missingFields
    .map(f => FIELD_QUESTIONS[f] || ('Puedes darme mas informacion sobre "' + f + '"?'))
    .filter(Boolean);
  ...
  switch (functionName) {
    case 'createEvent': {
      ...
      return {
        success: true,
        data:    event,
        message: 'Evento "' + event.title + '" anadido el ' + dateStr + ' a las ' + timeStr + '.',
      };
    }
    ...
    case 'schedulePlan': {
      ...
      return { success: true, data: { sessions, count: sessions.length }, message: summ };
    }
    case 'getEvents': {
...
async function generateConversationalResponse(userMessage, context, functionResult) {
  ...
  try {
    ...
    const systemPrompt = 'Eres Secretario IA, asistente de agenda. Hoy es ' + today + '.' +
      ' Hablas espanol de forma natural y concisa. Maximo 2-3 frases.' +
      (context.contextSummary ? '\n\nInfo usuario:\n' + context.contextSummary : '');
    const messages = [
      { role: 'system', content: systemPrompt },
      ...context.shortTermMemory.slice(-6).map(function(m) {
    ...
    const response = await groq.chat.completions.create({
      model: config.groq.model, messages, temperature: 0.75, max_tokens: 300,
    });
...
async function processChat(req, res, next) {
  ...
  try {
    ...
    if (intentResult.intent === INTENTS.SALUDO || intentResult.intent === INTENTS.OTRO) {
      ...
      return res.json({ success: true, response: resp, intent: intentResult.intent,
        confidence: intentResult.confidence, function_called: false, data: null });
    }

    // Datos faltantes -> preguntar
    if (intentResult.requires_data && intentResult.missing_fields.length > 0) {
      ...
      await saveMessage(userId, 'user', message, {
        intent: intentResult.intent, missing_fields: intentResult.missing_fields,
      ...
      return res.json({ success: true, response: question, intent: intentResult.intent,
        confidence: intentResult.confidence, missing_fields: intentResult.missing_fields,
        requires_more_info: true, function_called: false, data: null });
    }
      ...
      try {
        ...
        if (functionCall) {
          ...
          functionResult = await executeFunctionCall(
            functionCall.functionName, functionCall.arguments, userId, email, name
          );
    ...
    await saveMessage(userId, 'user', message, {
      intent: intentResult.intent, extracted_data: intentResult.extracted_data,
    });
    await saveMessage(userId, 'assistant', assistantResponse, {
      functionCalled: functionResult ? 'yes' : 'none',
    });
    await extractAndUpdateMemory(userId, message, intentResult.intent, intentResult.extracted_data);

    return res.json({
      success: true, response: assistantResponse, intent: intentResult.intent,
      confidence: intentResult.confidence, function_called: !!functionResult,
  ...
  if (pendingIntent && intentResult.intent === INTENTS.CONFIRMAR) {
    try {
      ...
      if (functionCall) {
        ...
        await saveMessage(userId, 'assistant', resp);
        return res.json({ success: true, response: resp, intent: 'confirmar',
