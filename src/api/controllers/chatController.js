...
// Fixed: context-aware classification, no "Hola" fallback mid-conversation, stable history
'use strict';
...
const FAST_INTENTS = {
  confirmar: /^(si|si|ok|vale|va|perfecto|adelante|claro|hazlo|confirma|correcto)$/i,
  cancelar: /^(no|cancela|olvidalo|olvidalo|mejor no|para|stop)$/i,
...
async function executeFunctionCall(functionName, args, userId, userEmail, userName) {
  switch (functionName) {
    case 'createEvent': {
      ...
      return {
        success: true, data: event, action_type: 'event_created',
    ...
    case 'createTrainingPlan': {
      ...
      return {
        success: true, data: plan, action_type: 'plan_created',
        message: formatPlanResponse(plan), requiresSchedule: true, planId: plan.id,
    ...
    case 'schedulePlan': {
      ...
      return {
        success: true, data: { sessions, count: sessions.length }, action_type: 'plan_scheduled',
        message: summ, summary: { sessions_count: sessions.length }
      };
    }
    case 'getEvents': {
    ...
    case 'updateEvent': {
      ...
      return {
        success: true, data: event, action_type: 'event_updated',
    ...
    case 'deleteEvent': {
      ...
      return {
        success: true, data: result, action_type: 'event_deleted',
...
function buildSystemPrompt(userContext) {
  ...
  p += ' NUNCA digas "Hola" ni "en que puedo ayudarte" si ya hay una conversacion activa.';
  p += ' NUNCA repitas el mensaje del usuario ni ignores el contexto previo.';
...
// Helper: detect if conversation has prior turns
function isConversationActive(shortTermMemory) {
  return shortTermMemory && shortTermMemory.length > 0;
}
...
async function generateConversationalResponse(userMessage, userContext, functionResult) {
  ...
  // FIX: context-aware fallback when no Groq key
  if (!config.groq.apiKey) {
    if (functionResult && functionResult.message) return functionResult.message;
    if (isConversationActive(userContext.shortTermMemory)) {
      return 'No entendi bien. Puedes explicarme que necesitas?';
    }
  ...
  try {
    ...
    // FIX: use last 6 turns for better context retention
    const recentHistory = userContext.shortTermMemory.slice(-6).map(function(m) {
    ...
    // FIX: context-aware error fallback -- never greet mid-conversation
    if (functionResult && functionResult.message) return functionResult.message;
    if (isConversationActive(userContext.shortTermMemory)) {
      return 'Lo siento, hubo un problema. Puedes repetir lo que necesitas?';
    }
    return 'Hola, en que puedo ayudarte?';
  }
}
...
async function processChat(req, res, next) {
  ...
  try {
    // FIX: always fetch context FIRST, then classify WITH history
    // The parallel optimization was losing context -- history must be available for classification
    const userContext = await buildUserContext(userId, message);
    const shortTermMemory = userContext.shortTermMemory || [];

    // Try fast regex classify first (0 API calls for simple inputs)
    const fastResult = fastClassify(message);

    // FIX: always pass full history to classifier so it understands follow-up messages like "como?" or "4 semanas"
    const finalIntent = fastResult
      ? fastResult
      : await classifyIntent(message, shortTermMemory);
    ...
    if (finalIntent.intent === INTENTS.SALUDO || finalIntent.intent === INTENTS.OTRO) {
      ...
      await Promise.all([
        saveMessage(userId, 'user', message, { intent: finalIntent.intent }),
        saveMessage(userId, 'assistant', resp)
      ]);
      setImmediate(function() {
        extractAndUpdateMemory(userId, message, finalIntent.intent, finalIntent.extracted_data).catch(function() {});
      });
      return res.json({
        success: true, response: resp, intent: finalIntent.intent,
        confidence: finalIntent.confidence, function_called: false, data: null,
        suggestions: FOLLOW_UP_SUGGESTIONS[finalIntent.intent] || []
      });
    }
    ...
    if (finalIntent.requires_data && finalIntent.missing_fields && finalIntent.missing_fields.length > 0) {
      ...
      await Promise.all([
        saveMessage(userId, 'user', message, {
          ...
          missing_fields: finalIntent.missing_fields,
          extracted_data: finalIntent.extracted_data
        }),
        saveMessage(userId, 'assistant', question, {
          pending_intent: finalIntent.intent,
          pending_data: finalIntent.extracted_data,
          missing_fields: finalIntent.missing_fields
        }),
      ]);
      return res.json({
        success: true, response: question, intent: finalIntent.intent,
        confidence: finalIntent.confidence, missing_fields: finalIntent.missing_fields,
        requires_more_info: true, function_called: false, data: null, suggestions: []
      });
    }
    ...
    if (actionableIntents.has(finalIntent.intent)) {
      try {
        const functionCall = await processFunctionCall(finalIntent.intent, message, finalIntent.extracted_data, shortTermMemory, userId);
        if (functionCall) {
    ...
    return res.json({
      success: true, response: assistantResponse, intent: finalIntent.intent,
      confidence: finalIntent.confidence, function_called: !!functionResult,
      action_type: actionType, summary: responseSummary,
      data: (functionResult && functionResult.data) || null,
      suggestions: actionType !== 'error' ? (FOLLOW_UP_SUGGESTIONS[finalIntent.intent] || []) : []
    });
  } catch (err) {
    next(err);
  }
...
async function handleConfirmOrCancel(intentResult, message, userId, userEmail, userName, userContext, res) {
  ...
  if (pendingIntent && intentResult.intent === INTENTS.CONFIRMAR) {
    try {
      ...
      if (functionCall) {
        ...
        return res.json({
          success: true, response: resp, intent: 'confirmar', function_called: true,
          data: fr.data || null, action_type: fr.action_type, summary: fr.summary || null,
  ...
  // FIX: context-aware fallback for confirm without pending action
  const resp2 = isConversationActive(history)
    ? 'Confirmas que exactamente? Cuentame lo que quieres hacer.'
...
async function getMemoryProfile(req, res, next) {
  try {
    res.json({ success: true, memory: await memoryGetProfile(req.user.id) });
  } catch (err) {
    next(err);
  }
}
