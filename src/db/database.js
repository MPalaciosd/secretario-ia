...
async function runMigrations(client) {
  try {
    ...
    // ── Memory tables ─────────────────────────────────────────────────────────
    // user_memory: long-term preferences, habits, goals, restrictions
    await client.query(`
    ...
    // user_habits: structured habit tracking table
    // Tracks frequency, last_seen, schedule patterns
    await client.query(`
    ...
    // user_preferences: typed key-value preference store
    await client.query(`
    ...
    // memory_embeddings: standalone embedding cache for semantic search
    await client.query(`
    ...
    for (const sql of alterColumns) {
      await client.query(sql).catch(function() {});
    }
    ...
    for (const idx of indexes) {
      await client.query(idx).catch(function(err) {
