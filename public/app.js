// SECRETARIO IA — React Frontend v3.1 (UX improved)
...
function AuthProvider({ children }) {
  ...
  return React.createElement(AuthContext.Provider, { value: { user, login, register, logout, loading, isPro } }, children);
}
...
const Icon = function({ name, size, className }) {
  ...
  const icons = {
    ...
    plus: 'M12 5v14M5 12h14', send: 'M22 2L11 13M22 2L15 22l-4-9-9-4 20-7z',
    user: 'M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2M12 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z',
    star: 'M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z',
    check: 'M20 6L9 17l-5-5', logout: 'M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 0 1-3 3H6a3 3 0 0 1-3-3V7a3 3 0 0 1 3-3h4a3 3 0 0 1 3 3v1',
    clock: 'M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2zM12 6v6l4 2', trash: 'M3 6h18M8 6V4h8v2M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6',
    lightning: 'M13 2L3 14h9l-1 8 10-12h-9l1-8z', alert: 'M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z',
    chevronDown: 'M6 9l6 6 6-6', x: 'M18 6L6 18M6 6l12 12'
  };
  ...
  return React.createElement('div', { className: 'fixed inset-0 z-50 flex items-center justify-center modal-overlay bg-black/30', onClick: function(e) { if (e.target === e.currentTarget) onClose(); } },
    React.createElement('div', { className: 'bg-cream rounded-2xl p-8 w-full max-w-md shadow-2xl fade-in border border-border' },
      ...
      React.createElement('div', { className: 'flex bg-panel rounded-xl p-1 mb-6' },
        ['login','register'].map(function(m) { return React.createElement('button', { key: m, onClick: function() { setMode(m); setError(''); }, className: 'flex-1 py-2 rounded-lg text-sm font-medium transition-all ' + (mode === m ? 'bg-white text-wine shadow-sm' : 'text-wine-light hover:text-wine') }, m === 'login' ? 'Iniciar sesion' : 'Registrarse'); })
      ),
      React.createElement('form', { onSubmit: handleSubmit, className: 'space-y-4' },
        mode === 'register' && React.createElement('div', null,
          ...
          React.createElement('input', { type: 'text', value: name, onChange: function(e) { setName(e.target.value); }, placeholder: 'Tu nombre', className: 'w-full bg-panel border border-border rounded-xl px-4 py-3 text-wine placeholder-wine/40 focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all' })
        ),
        React.createElement('div', null,
          ...
          React.createElement('input', { type: 'email', value: email, onChange: function(e) { setEmail(e.target.value); }, placeholder: 'tu@email.com', className: 'w-full bg-panel border border-border rounded-xl px-4 py-3 text-wine placeholder-wine/40 focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all' })
        ),
  ...
  return React.createElement('aside', { className: 'w-56 bg-sidebar h-full flex flex-col border-r border-border flex-shrink-0' },
    React.createElement('div', { className: 'p-5 border-b border-border' },
      React.createElement('div', { className: 'flex items-center gap-3' },
        ...
        React.createElement('div', null, React.createElement('h1', { className: 'font-bold text-wine text-sm leading-tight' }, 'Secretario IA'), React.createElement('p', { className: 'text-wine/50 text-xs' }, 'v3.1'))
      )
    ),
    ...
    React.createElement('div', { className: 'p-3 border-t border-border space-y-2' },
      user && isPro() && React.createElement('div', { className: 'flex items-center gap-2 px-3 py-2 bg-accent/10 rounded-xl' },
      ...
      user ? React.createElement('div', { className: 'space-y-1' },
        ...
        React.createElement('button', { onClick: logout, className: 'w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs text-wine/60 hover:bg-wine/10 hover:text-wine transition-all' },
          React.createElement(Icon, { name: 'logout', size: 14 }), 'Cerrar sesion')
...
function ActionCard({ actionType, summary }) {
  ...
  var configs = {
    event_created: { icon: '📅', label: 'Evento creado', color: 'bg-green-50 border-green-200 text-green-800' },
    plan_created:  { icon: '🏋️', label: 'Plan creado',   color: 'bg-blue-50 border-blue-200 text-blue-800' },
    plan_scheduled:{ icon: '📆', label: 'Sesiones programadas', color: 'bg-purple-50 border-purple-200 text-purple-800' },
    event_updated: { icon: '✏️', label: 'Evento actualizado', color: 'bg-amber-50 border-amber-200 text-amber-800' },
    event_deleted: { icon: '🗑️', label: 'Evento eliminado', color: 'bg-red-50 border-red-200 text-red-700' },
    events_listed: { icon: '🔍', label: 'Tu agenda',      color: 'bg-wine/5 border-wine/20 text-wine' },
  };
  ...
  return React.createElement('div', { className: 'mt-2 p-2.5 rounded-xl border text-xs ' + cfg.color },
    React.createElement('div', { className: 'flex items-center gap-1.5 font-semibold mb-1.5' },
      React.createElement('span', null, cfg.icon), cfg.label
    ),
    fields.length > 0 && React.createElement('div', { className: 'space-y-0.5 opacity-80' },
      fields.slice(0, 4).map(function(e, i) {
        return React.createElement('div', { key: i }, String(e[0]).replace(/_/g, ' ') + ': ' + String(e[1]));
      })
    )
  );
}
...
function ChatPanel() {
  ...
  var sendMessage = async function(text) {
    ...
    if (!isPro() && msgCount >= FREE_LIMIT) { show('Limite semanal alcanzado. Actualiza a PRO.', 'error'); return; }
    setInput('');
    if (inputRef.current) { inputRef.current.style.height = 'auto'; }
    setMessages(function(prev) { return prev.concat([{ role: 'user', content: msgText }]); });
    setLoading(true);
    setMsgCount(function(c) { return c + 1; });
    // Detect confirm/cancel for pending action UI
    var lower = msgText.toLowerCase();
    if (pendingAction && /^(si|sí|ok|vale|adelante|confirma|yes)$/i.test(lower.trim())) setPendingAction(null);
    ...
    try {
      ...
      // Track pending confirm/cancel state
      if (data.requires_more_info && data.missing_fields && data.missing_fields.length > 0) {
        setPendingAction({ intent: data.intent, question: data.response });
      } else if (data.action_type && data.action_type !== 'error') {
  ...
  var intentBadge = function(intent) {
    var map = {
      crear_evento: { label: 'Evento', color: 'bg-wine/15 text-wine' },
      ...
      eliminar:     { label: 'Eliminar', color: 'bg-red-100 text-red-600' },
    };
  ...
  return React.createElement('div', { className: 'flex-1 flex flex-col bg-cream overflow-hidden' },
    ...
    React.createElement('div', { className: 'flex-1 overflow-hidden relative' },
            ...
            React.createElement('div', { className: 'max-w-[82%]' },
              ...
              React.createElement('div', {
                className: (msg.role === 'user'
                  ? 'bg-wine text-cream rounded-2xl rounded-br-md'
                  : msg.error
                    ? 'bg-red-50 border border-red-200 text-red-700 rounded-2xl rounded-bl-md'
                    : 'bg-panel text-wine rounded-2xl rounded-bl-md') + ' px-4 py-3 text-sm leading-relaxed'
              },
                msg.role === 'user'
              ...
              msg.role === 'assistant' && msg.requires_more_info && i === messages.length - 1 &&
              ...
              msg.role === 'assistant' && msg.suggestions && msg.suggestions.length > 0 && i === messages.length - 1 &&
                React.createElement('div', { className: 'flex flex-wrap gap-1.5 mt-2' },
                  msg.suggestions.map(function(s, si) {
                    return React.createElement('button', { key: si, onClick: function() { sendMessage(s); }, className: 'text-[11px] bg-cream border border-border text-wine px-2.5 py-1 rounded-lg hover:border-accent/60 hover:bg-panel transition-all' }, s);
                  })
                )
            )
          );
        ...
        loading && React.createElement('div', { className: 'flex justify-start fade-in' },
          React.createElement('div', { className: 'bg-panel rounded-2xl rounded-bl-md px-4 py-3 flex items-center gap-2' },
            React.createElement('div', { className: 'flex gap-1 items-center h-5' },
              [1,2,3].map(function(n) { return React.createElement('div', { key: n, className: 'w-2 h-2 bg-wine/40 rounded-full typing-dot' }); })
            ),
      ...
      showScrollBtn && React.createElement('button', {
        onClick: function() { scrollToBottom(true); },
        className: 'absolute bottom-4 right-4 w-9 h-9 bg-wine text-cream rounded-full shadow-lg flex items-center justify-center hover:bg-wine/80 transition-all fade-in z-10'
    ...
    !isPro() && msgCount >= FREE_LIMIT - 3 && React.createElement('div', { className: 'mx-6 mb-2 p-3 bg-wine/5 border border-wine/20 rounded-xl' },
      React.createElement('p', { className: 'text-xs text-wine/70 text-center' },
        ...
        React.createElement('a', { href: '#', onClick: function(e) { e.preventDefault(); }, className: 'font-semibold text-accent underline' }, 'Actualiza a PRO')
      )
    ),
    React.createElement('div', { className: 'px-6 pb-6 pt-2' },
      React.createElement('div', { className: 'flex items-end gap-3 bg-panel rounded-2xl border border-border p-3 focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/20 transition-all' + (isLimited ? ' opacity-50' : '') },
        React.createElement('textarea', {
          ...
          onKeyDown: handleKey,
          placeholder: isLimited ? 'Limite semanal alcanzado. Actualiza a PRO.' : 'Escribe algo... (Enter para enviar, Shift+Enter para nueva linea)',
        ...
        React.createElement('div', { className: 'flex flex-col items-end gap-2 flex-shrink-0' },
          ...
          React.createElement('button', {
            ...
            className: 'w-9 h-9 rounded-xl gradient-accent flex items-center justify-center text-white hover:opacity-90 disabled:opacity-40 transition-all'
...
function CalendarView() {
  ...
  if (!user) return React.createElement('div', { className: 'flex-1 flex items-center justify-center bg-cream' },
    React.createElement('div', { className: 'text-center space-y-3' },
      React.createElement('div', { className: 'w-16 h-16 rounded-2xl gradient-accent flex items-center justify-center mx-auto' }, React.createElement(LogoIcon, { size: 36, className: 'text-white' })),
  ...
  return React.createElement('div', { className: 'flex-1 flex overflow-hidden bg-cream' },
    React.createElement('div', { className: 'flex-1 flex flex-col p-6 overflow-auto' },
      React.createElement('div', { className: 'flex items-center justify-between mb-6' },
        ...
        React.createElement('div', { className: 'flex gap-2' },
          ...
          React.createElement('button', { onClick: function() { setShowForm(true); }, className: 'flex items-center gap-2 px-4 py-2 text-sm gradient-accent text-white rounded-xl font-medium hover:opacity-90 transition-all' },
      ...
      React.createElement('div', { className: 'grid grid-cols-7 gap-1 flex-1' },
        Array.from({ length: totalCells }, function(_, i) {
          ...
          return React.createElement('div', { key: i, onClick: function() { if (isValid) setSelectedDate(new Date(currentYear, currentMonth, day)); },
            className: 'min-h-[80px] p-2 rounded-xl border transition-all cursor-pointer ' + (!isValid ? 'opacity-0 pointer-events-none' : isSelected ? 'bg-wine border-wine text-cream' : isToday ? 'bg-accent/10 border-accent/40' : 'bg-panel/60 border-transparent hover:border-border hover:bg-panel')
          },
            isValid && React.createElement('div', null,
              ...
              React.createElement('div', { className: 'mt-1 space-y-0.5' },
                dayEvents.slice(0, 2).map(function(ev, idx) {
                  return React.createElement('div', { key: idx, className: 'text-[10px] px-1.5 py-0.5 rounded-md truncate font-medium ' + (isSelected ? 'bg-white/20 text-cream' : (eventTypeColors[ev.event_type] || 'bg-wine/60') + ' text-white') }, ev.title);
                }),
    ...
    React.createElement('div', { className: 'w-72 bg-panel border-l border-border flex flex-col overflow-hidden' },
      React.createElement('div', { className: 'p-4 border-b border-border' },
        React.createElement('h3', { className: 'font-bold text-wine' }, selectedDate.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' })),
        React.createElement('p', { className: 'text-wine/50 text-xs mt-0.5' }, selectedDayEvents.length === 0 ? 'Sin eventos' : selectedDayEvents.length + ' evento' + (selectedDayEvents.length > 1 ? 's' : ''))
      ),
      React.createElement('div', { className: 'flex-1 overflow-auto p-4 space-y-2 chat-scroll' },
        selectedDayEvents.length === 0
          ? React.createElement('div', { className: 'text-center py-8 text-wine/40' },
              ...
              React.createElement('p', { className: 'text-sm' }, 'Dia libre')
            )
          : selectedDayEvents.map(function(ev) {
              return React.createElement('div', { key: ev.id, className: 'event-card bg-cream rounded-xl p-3 border border-border group' },
                React.createElement('div', { className: 'flex items-start justify-between' },
                  ...
                  React.createElement('button', { onClick: function() { deleteEvent(ev.id); }, className: 'opacity-0 group-hover:opacity-100 p-1 hover:bg-red-50 rounded-lg text-red-400 transition-all' },
...
function PricingPage({ onShowAuth }) {
  ...
  const { show, ToastEl } = useToast();
  const plans = [
    { id: 'free', name: 'Gratuito', price: '0', period: 'para siempre', description: 'Empieza a organizar tu agenda con IA', color: 'border-border', badge: null,
      features: [{ok:true,text:'20 mensajes de IA por semana'},{ok:true,text:'20 eventos en el calendario'},{ok:true,text:'Clasificacion de intenciones'},{ok:true,text:'Creacion basica de eventos'},{ok:false,text:'Planes de entrenamiento IA'},{ok:false,text:'Scheduler automatico'},{ok:false,text:'Memoria personalizada'},{ok:false,text:'Emails de confirmacion'}],
      cta: 'Empezar gratis', ctaStyle: 'border-2 border-border text-wine hover:bg-panel' },
    { id: 'pro', name: 'PRO', price: '9', period: '/ mes', description: 'Para personas que quieren el maximo de productividad', color: 'border-accent', badge: 'Mas popular',
      features: [{ok:true,text:'IA ilimitada'},{ok:true,text:'Eventos ilimitados'},{ok:true,text:'Planes de entrenamiento con IA'},{ok:true,text:'Scheduler inteligente automatico'},{ok:true,text:'Memoria personalizada a largo plazo'},{ok:true,text:'Emails de confirmacion'},{ok:true,text:'Analisis de disponibilidad'},{ok:true,text:'Anti-solapamiento de eventos'}],
      cta: 'Suscribirse', ctaStyle: 'gradient-accent text-white hover:opacity-90' },
    { id: 'premium', name: 'Premium', price: '19', period: '/ mes', description: 'Para equipos y usuarios avanzados', color: 'border-wine/40', badge: 'Proximamente',
      features: [{ok:true,text:'Todo lo del plan PRO'},{ok:true,text:'Multiples calendarios'},{ok:true,text:'Integracion Google Calendar'},{ok:true,text:'Recordatorios SMS/WhatsApp'},{ok:true,text:'Reportes semanales'},{ok:true,text:'Hasta 3 usuarios'}],
  ...
  const handleSubscribe = async function(planId) {
    ...
    setLoading(planId);
    try { var data = await api.get('/api/stripe/checkout'); if (data.url) window.open(data.url, '_blank'); }
    catch(err) { show('Error al procesar el pago.', 'error'); } finally { setLoading(''); }
  };
        ...
        return React.createElement('div', { key: plan.id, className: 'relative bg-panel rounded-2xl p-6 border-2 ' + plan.color + ' flex flex-col ' + (plan.id === 'pro' ? 'shadow-xl scale-105' : '') },
          ...
          React.createElement('div', { className: 'mb-5' },
            ...
            React.createElement('div', { className: 'flex items-baseline gap-1 mb-2' },
              React.createElement('span', { className: 'text-4xl font-bold text-wine' }, plan.price + String.fromCharCode(8364)),
          ...
          React.createElement('ul', { className: 'flex-1 space-y-2 mb-5' },
            plan.features.map(function(f, i) {
              return React.createElement('li', { key: i, className: 'flex items-start gap-2' },
                React.createElement('div', { className: 'w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ' + (f.ok ? 'bg-accent/20 text-accent' : 'bg-wine/10 text-wine/30') },
                  f.ok ? React.createElement(Icon, { name: 'check', size: 10 }) : React.createElement('span', { style: { fontSize: '10px' } }, 'x')
                ),
                React.createElement('span', { className: 'text-sm ' + (f.ok ? 'text-wine' : 'text-wine/40') }, f.text)
              );
            })
          ),
          React.createElement('button', { onClick: function() { handleSubscribe(plan.id); }, disabled: loading === plan.id || (plan.id === 'pro' && isPro()), className: 'w-full py-3 rounded-xl font-semibold text-sm transition-all disabled:opacity-60 ' + plan.ctaStyle },
            loading === plan.id ? 'Procesando...' : (plan.id === 'pro' && isPro() ? 'Plan activo' : plan.cta)
          )
        );
      })
    ),
...
function App() {
  ...
  useEffect(function() {
    ...
    if (params.get('payment') === 'success') {
      setTimeout(function() { alert('Suscripcion activada! Bienvenido a PRO.'); }, 500);
  ...
  var renderView = function() {
    ...
    if (view === 'chat') return React.createElement(ChatPanel);
    if (view === 'pricing') return React.createElement(PricingPage, { onShowAuth: function() { setShowAuth(true); } });
    return React.createElement(CalendarView);
  };
  return React.createElement('div', { className: 'h-screen flex overflow-hidden' },
    React.createElement(Sidebar, { view, setView, onShowAuth: function() { setShowAuth(true); } }),
