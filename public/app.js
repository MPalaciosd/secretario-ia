...
function AuthProvider({ children }) {
  ...
  return React.createElement(AuthContext.Provider, 
    { value: { user, login, register, logout, loading, isPro } }, 
    children
  );
}
...
const Icon = ({ name, size = 18, className = '' }) => {
  const icons = {
    ...
    brain: 'M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10 10-4.5 10-10S17.5 2 12 2zm0 3c1.1 0 2 .9 2 2s-.9 2-2 2-2-.9-2-2 .9-2 2-2zm4 9c0 2.2-1.8 4-4 4s-4-1.8-4-4v-1h8v1z',
    clock: 'M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2zM12 6v6l4 2',
    trash: 'M3 6h18M8 6V4h8v2M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6',
    lightning: 'M13 2L3 14h9l-1 8 10-12h-9l1-8z'
  };
  const d = icons[name] || icons.star;
  const isStroke = !['brain'].includes(name);
...
function Toast({ message, type, onClose }) {
  ...
  const colors = { success: 'bg-sage text-white', error: 'bg-red-500 text-white', info: 'bg-wine text-cream' };
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', { className: 'bg-cream rounded-2xl p-8 w-full max-w-md shadow-2xl fade-in border border-border' },
      ...
      React.createElement('div', { className: 'text-center mb-8' },
        React.createElement('div', { className: 'w-14 h-14 rounded-2xl gradient-sage flex items-center justify-center mx-auto mb-4' },
          React.createElement(Icon, { name: 'brain', size: 28, className: 'text-white' })
        ),
      ...
      React.createElement('div', { className: 'flex bg-panel rounded-xl p-1 mb-6' },
        ['login', 'register'].map(m =>
          React.createElement('button', {
            ...
            className: `flex-1 py-2 rounded-lg text-sm font-medium transition-all ${mode === m ? 'bg-white text-wine shadow-sm' : 'text-wine-light hover:text-wine'}`
      ...
      React.createElement('form', { onSubmit: handleSubmit, className: 'space-y-4' },
        mode === 'register' && React.createElement('div', null,
          ...
          React.createElement('input', {
            ...
            className: 'w-full bg-panel border border-border rounded-xl px-4 py-3 text-wine placeholder-wine/40 focus:border-sage focus:ring-2 focus:ring-sage/20 transition-all'
          })
        ),
        ...
        error && React.createElement('p', { className: 'text-red-500 text-sm bg-red-50 px-3 py-2 rounded-lg' }, error),
        React.createElement('button', {
          type: 'submit', disabled: loading,
          className: 'w-full gradient-sage text-white py-3 rounded-xl font-semibold hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50 mt-2'
...
// SIDEBAR COMPONENT
// ═══════════════════════════════════════════════════════════════
function Sidebar({ view, setView, onShowAuth, onShowPricing }) {
  ...
  return React.createElement('aside', {
    ...
    React.createElement('div', { className: 'p-5 border-b border-border' },
      React.createElement('div', { className: 'flex items-center gap-3' },
        React.createElement('div', { className: 'w-9 h-9 rounded-xl gradient-sage flex items-center justify-center' },
          React.createElement(Icon, { name: 'brain', size: 20, className: 'text-white' })
        ),
        React.createElement('div', null,
    ...
    React.createElement('div', { className: 'p-3 border-t border-border space-y-2' },
      user && isPro() && React.createElement('div', {
        className: 'flex items-center gap-2 px-3 py-2 bg-sage/15 rounded-xl'
      },
        React.createElement(Icon, { name: 'lightning', size: 14, className: 'text-sage' }),
        React.createElement('span', { className: 'text-xs font-semibold text-sage' }, 'Plan PRO activo')
      ),
      ...
      user ? React.createElement('div', { className: 'space-y-1' },
        ...
        onClick: onShowAuth,
        className: 'w-full gradient-sage text-white py-2.5 rounded-xl text-sm font-medium hover:opacity-90 transition-all'
      }, 'Iniciar sesión')
    )
  );
}
...
function CalendarView() {
  ...
  const firstDay = (getFirstDayOfMonth(currentYear, currentMonth) + 6) % 7; // Mon-start
  ...
  const eventTypeColors = {
    general: 'bg-sage/80', medico: 'bg-red-400', trabajo: 'bg-blue-400',
    personal: 'bg-purple-400', deporte: 'bg-orange-400', reunion: 'bg-yellow-500'
  };
  ...
  if (!user) return React.createElement('div', {
    ...
    React.createElement('div', { className: 'text-center space-y-3' },
      React.createElement('div', { className: 'w-16 h-16 rounded-2xl gradient-sage flex items-center justify-center mx-auto' },
        React.createElement(Icon, { name: 'calendar', size: 32, className: 'text-white' })
      ),
      ...
      React.createElement('div', { className: 'flex items-center justify-between mb-6' },
        ...
        React.createElement('div', { className: 'flex gap-2' },
          ...
          React.createElement('button', {
            ...
            className: 'flex items-center gap-2 px-4 py-2 text-sm gradient-sage text-white rounded-xl font-medium hover:opacity-90 transition-all'
        ...
        Array.from({ length: totalCells }, (_, i) => {
          ...
          return React.createElement('div', {
            ...
            className: `min-h-[80px] p-2 rounded-xl border transition-all cursor-pointer ${
              ...
              isToday ? 'bg-sage/20 border-sage' :
              'bg-panel/60 border-transparent hover:border-border hover:bg-panel'
            }`
          },
            isValid && React.createElement('div', null,
              React.createElement('span', {
                className: `text-sm font-semibold ${isSelected ? 'text-cream' : isToday ? 'text-sage-dark' : 'text-wine'}`
              }, day),
              React.createElement('div', { className: 'mt-1 space-y-0.5' },
                dayEvents.slice(0, 2).map((ev, idx) =>
                  React.createElement('div', {
                    key: idx,
                    className: `text-[10px] px-1.5 py-0.5 rounded-md truncate font-medium ${isSelected ? 'bg-white/20 text-cream' : (eventTypeColors[ev.event_type] || 'bg-sage/80') + ' text-white'}`
                  }, ev.title)
                ),
              ...
              React.createElement('div', { key: ev.id, className: 'event-card bg-cream rounded-xl p-3 border border-border group' },
                React.createElement('div', { className: 'flex items-start justify-between' },
                  React.createElement('div', { className: 'flex-1 min-w-0' },
                    React.createElement('div', {
                      className: `inline-block text-[10px] px-2 py-0.5 rounded-full font-semibold mb-1 ${eventTypeColors[ev.event_type] || 'bg-sage/80'} text-white`
                    }, ev.event_type),
    ...
    showForm && React.createElement('div', {
      ...
      React.createElement('div', { className: 'bg-cream rounded-2xl p-6 w-full max-w-md shadow-2xl fade-in border border-border' },
        ...
        React.createElement('form', { onSubmit: createEvent, className: 'space-y-4' },
          React.createElement('input', {
            required: true, placeholder: 'Título del evento',
            value: newEvent.title, onChange: e => setNewEvent(p => ({ ...p, title: e.target.value })),
            className: 'w-full bg-panel border border-border rounded-xl px-4 py-3 text-wine placeholder-wine/40 focus:border-sage transition-all'
          }),
          React.createElement('div', { className: 'grid grid-cols-2 gap-3' },
            React.createElement('input', {
              required: true, type: 'date',
              value: newEvent.date, onChange: e => setNewEvent(p => ({ ...p, date: e.target.value })),
              className: 'bg-panel border border-border rounded-xl px-3 py-3 text-wine focus:border-sage transition-all'
            }),
            React.createElement('input', {
          ...
          React.createElement('div', { className: 'grid grid-cols-2 gap-3' },
            React.createElement('input', {
              ...
              value: newEvent.duration_minutes, onChange: e => setNewEvent(p => ({ ...p, duration_minutes: parseInt(e.target.value) })),
              className: 'bg-panel border border-border rounded-xl px-3 py-3 text-wine focus:border-sage transition-all'
            }),
            React.createElement('select', {
              value: newEvent.event_type, onChange: e => setNewEvent(p => ({ ...p, event_type: e.target.value })),
              className: 'bg-panel border border-border rounded-xl px-3 py-3 text-wine focus:border-sage transition-all'
            },
          ...
          React.createElement('div', { className: 'flex gap-3 pt-2' },
            ...
            React.createElement('button', {
              type: 'submit',
              className: 'flex-1 gradient-sage text-white py-3 rounded-xl font-medium hover:opacity-90 transition-all'
            }, 'Crear evento')
          )
        )
      )
...
function ChatPanel() {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', content: '¡Hola! Soy tu secretario IA. Puedo ayudarte a:\n\n📅 Crear y organizar eventos\n🏋️ Planificar entrenamientos\n🔍 Consultar tu agenda\n\n¿Qué necesitas hoy?' }
  ]);
  ...
  const sendMessage = async () => {
    ...
    if (!isPro() && msgCount >= FREE_LIMIT) {
      show('Has alcanzado el límite del plan gratuito. Actualiza a PRO para continuar.', 'error');
      return;
    }
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: 'Lo siento, hubo un error. Inténtalo de nuevo.', error: true }]);
  ...
  const intentBadge = (intent) => {
    const map = {
      crear_evento: { label: 'Evento', color: 'bg-sage/20 text-sage-dark' },
      crear_plan: { label: 'Plan', color: 'bg-blue-100 text-blue-700' },
      consultar: { label: 'Consulta', color: 'bg-yellow-100 text-yellow-700' },
  ...
  if (!user) return React.createElement('div', {
    ...
    React.createElement('div', { className: 'w-20 h-20 rounded-3xl gradient-sage flex items-center justify-center' },
      React.createElement(Icon, { name: 'chat', size: 40, className: 'text-white' })
    ),
  ...
  return React.createElement('div', { className: 'flex-1 flex flex-col bg-cream overflow-hidden' },
    ...
    React.createElement('div', { className: 'px-6 py-4 border-b border-border flex items-center justify-between bg-cream' },
      React.createElement('div', { className: 'flex items-center gap-3' },
        React.createElement('div', { className: 'w-9 h-9 rounded-xl gradient-sage flex items-center justify-center' },
          React.createElement(Icon, { name: 'brain', size: 18, className: 'text-white' })
        ),
        React.createElement('div', null,
          React.createElement('h2', { className: 'font-bold text-wine text-sm' }, 'Secretario IA'),
          React.createElement('div', { className: 'flex items-center gap-1.5' },
            React.createElement('div', { className: 'w-2 h-2 rounded-full bg-sage animate-pulse' }),
    ...
    React.createElement('div', {
      ref: chatRef,
      className: 'flex-1 overflow-auto px-6 py-4 space-y-4 chat-scroll'
    },
      messages.map((msg, i) =>
        React.createElement('div', { key: i, className: `flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} fade-in` },
          React.createElement('div', {
            ...
            msg.intent && intentBadge(msg.intent) && React.createElement('div', {
            ...
            React.createElement('p', { className: 'whitespace-pre-line' }, formatContent(msg.content))
          )
        )
      ),
        ...
        React.createElement('div', { className: 'bg-panel rounded-2xl rounded-bl-md px-4 py-3' },
          React.createElement('div', { className: 'flex gap-1 items-center h-5' },
            [1,2,3].map(n => React.createElement('div', {
              key: n, className: 'w-2 h-2 bg-wine/40 rounded-full typing-dot'
            }))
          )
        )
      )
      ...
      React.createElement('div', { className: 'flex flex-wrap gap-2' },
        suggestions.map((s, i) =>
          React.createElement('button', {
            ...
            className: 'text-xs bg-panel hover:bg-border text-wine px-3 py-1.5 rounded-xl transition-all border border-border hover:border-sage/50'
          }, s)
        )
      )
    ),
    ...
    // Free limit warning
    !isPro() && msgCount >= FREE_LIMIT - 2 && React.createElement('div', {
      ...
      React.createElement('p', { className: 'text-xs text-wine/70 text-center' },
        msgCount >= FREE_LIMIT
          ? '🔒 Límite alcanzado. '
          : `⚠️ Te quedan ${FREE_LIMIT - msgCount} mensajes. `,
        React.createElement('a', { href: '#pricing', className: 'font-semibold text-sage underline' }, 'Actualiza a PRO')
      )
    ),

    // Input area
    React.createElement('div', { className: 'px-6 pb-6 pt-2' },
      React.createElement('div', { className: 'flex items-end gap-3 bg-panel rounded-2xl border border-border p-3 focus-within:border-sage focus-within:ring-2 focus-within:ring-sage/20 transition-all' },
        React.createElement('textarea', {
          ref: inputRef,
          value: input, onChange: e => setInput(e.target.value), onKeyDown: handleKey,
          placeholder: isPro() ? 'Escribe algo... (Enter para enviar)' : `${FREE_LIMIT - msgCount} mensajes restantes...`,
        ...
        React.createElement('button', {
          onClick: sendMessage, disabled: loading || !input.trim() || (!isPro() && msgCount >= FREE_LIMIT),
          className: 'w-9 h-9 rounded-xl gradient-sage flex items-center justify-center text-white hover:opacity-90 disabled:opacity-40 transition-all flex-shrink-0'
...
function PricingPage({ onShowAuth }) {
  ...
  const plans = [
    {
      ...
      badge: null,
      features: [
        { ok: true, text: '10 mensajes de IA al día' },
      ...
      cta: 'Empezar gratis',
      ctaStyle: 'border-2 border-border text-wine hover:bg-panel'
    },
    {
      id: 'pro',
      name: 'PRO',
      ...
      ctaStyle: 'gradient-sage text-white hover:opacity-90'
    },
    {
      id: 'premium',
      name: 'Premium',
      ...
      cta: 'Lista de espera',
      ctaStyle: 'border-2 border-wine/30 text-wine hover:bg-wine/5'
    }
  ];
  ...
  const handleSubscribe = async (planId) => {
    ...
    if (planId === 'premium') {
      show('¡Anotado! Te avisamos cuando esté disponible 🚀', 'success');
      return;
    }
  ...
  return React.createElement('div', { className: 'flex-1 overflow-auto bg-cream p-8 chat-scroll' },
    ...
    React.createElement('div', { className: 'text-center max-w-2xl mx-auto mb-12' },
      React.createElement('div', { className: 'inline-flex items-center gap-2 bg-sage/15 text-sage-dark px-4 py-2 rounded-full text-sm font-semibold mb-4' },
        React.createElement(Icon, { name: 'star', size: 16 }),
        'Pricing transparente'
      ),
      React.createElement('h2', { className: 'text-4xl font-bold text-wine mb-4' }, 'Elige tu plan'),
      React.createElement('p', { className: 'text-wine/60 text-lg' },
        'Sin compromisos. Cancela cuando quieras. Primer mes garantizado.'
      )
    ),
        ...
        React.createElement('div', {
          ...
          plan.badge && React.createElement('div', {
            className: `absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-xs font-bold ${
              plan.badge === 'Más popular' ? 'bg-sage text-white' : 'bg-wine text-cream'
            }`
          }, plan.badge),
          ...
          React.createElement('ul', { className: 'flex-1 space-y-2.5 mb-6' },
            plan.features.map((f, i) =>
              React.createElement('li', { key: i, className: 'flex items-start gap-2.5' },
                React.createElement('div', {
                  className: `w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${f.ok ? 'bg-sage/20 text-sage' : 'bg-wine/10 text-wine/30'}`
                },
                  f.ok
    ...
    React.createElement('div', { className: 'max-w-2xl mx-auto' },
      ...
      React.createElement('div', { className: 'space-y-3' },
        [
          ...
          ['¿Cómo funciona el plan gratuito?', 'Tienes 10 mensajes de IA al día y hasta 20 eventos. Sin tarjeta de crédito.'],
...
function App() {
  ...
  if (loading) return React.createElement('div', {
    className: 'h-screen flex items-center justify-center bg-cream'
  },
    React.createElement('div', { className: 'text-center space-y-3' },
      React.createElement('div', { className: 'w-14 h-14 rounded-2xl gradient-sage flex items-center justify-center mx-auto' },
        React.createElement(Icon, { name: 'brain', size: 28, className: 'text-white animate-pulse' })
      ),
  ...
  return React.createElement('div', { className: 'h-screen flex overflow-hidden' },
    ...
    React.createElement('main', { className: 'flex-1 flex flex-col overflow-hidden' },
      ...
      React.createElement('div', { className: 'flex items-center px-4 py-3 border-b border-border bg-cream md:hidden' },
        React.createElement('button', {
          onClick: () => setSidebarOpen(o => !o),
          className: 'p-2 rounded-lg hover:bg-panel text-wine'
        }, '☰')
      ),
